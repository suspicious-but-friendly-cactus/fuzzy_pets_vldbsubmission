...
check_frodoPIR_params(const size_t db_entry_count, const size_t mat_element_bitlen)
{
  return check_frodoPIR_param_correctness(db_entry_count, mat_element_bitlen) && // First check if Eq. 8 of https://ia.cr/2022/981 holds
         (((db_entry_count == (1ul << 10)) && (mat_element_bitlen == 10)) ||     // Database has 2^10 rows s.t. each row can be of arbitrary byte length
          ((db_entry_count == (1ul << 12)) && (mat_element_bitlen == 10)) ||     // Database has 2^12 rows s.t. each row can be of arbitrary byte length
          ((db_entry_count == (1ul << 14)) && (mat_element_bitlen == 10)) ||     // Database has 2^14 rows s.t. each row can be of arbitrary byte length
