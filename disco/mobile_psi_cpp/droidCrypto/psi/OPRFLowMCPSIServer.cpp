...
namespace droidCrypto {
    ...
    void OPRFLowMCPSIServer::Setup(std::vector<block> &elements) {
        ...
        Log::v("PSI", "Setup Total Time:\t%fsec,\nTime no transm:\t%fsec,\nSetup Comm: %fMiB sent, %fMiB recv\n",
               (enc_time+cf_time+trans_time).count(),(enc_time+cf_time+trans_time).count(), channel_.getBytesSent()/1024.0/1024.0, channel_.getBytesRecv()/1024.0/1024.0);
