...
namespace droidCrypto {
    ...
    void OPRFLowMCPSIClient::Base(size_t num_elements) {
        channel_.clearStats();
        Log::v("PSI", "num_elements %d", num_elements);
    ...
    std::vector<size_t> OPRFLowMCPSIClient::Online(std::vector<block> &elements) {
        ...
        //std::cout << "Vector Size: " <<result.size() << ", BitVector Size: " << result[0].size() <<"\n";
        // std::string time = "Time:\n\t OT:   " + std::to_string(circ_.timeBaseOT.count());
        // time += ",\n\t OTe:  " + std::to_string(circ_.timeOT.count());
        // time += ",\n\t Send: " + std::to_string(circ_.timeSendGC.count());
        // time += ",\n\t Eval: " + std::to_string(circ_.timeEval.count());
        // time += ";\n\t Total:" + std::to_string((circ_.timeBaseOT+circ_.timeOT+circ_.timeEval+circ_.timeSendGC).count());
        // droidCrypto::Log::v("GC", "Online OPRF + Base: %s", time.c_str());
        droidCrypto::Log::v("GC", "Online: Sent: %zu, Recv: %zu", channel_.getBytesSent(), channel_.getBytesRecv());
        ...
        Log::v("PSI", "Online Time Total: %fsec", (circ_.timeBaseOT+circ_.timeOT+circ_.timeEval+circ_.timeSendGC+inter_time).count());

        return res;
    }

    void OPRFLowMCPSIClient::OnlineOPRF(std::vector<block> &elements, uint8_t * ptr) {
         size_t num_client_elements = elements.size();
        //do GC evaluation
        ...
        Log::v("PSI", "Online Time Total: %fsec", (circ_.timeBaseOT+circ_.timeOT+circ_.timeEval+circ_.timeSendGC).count());
