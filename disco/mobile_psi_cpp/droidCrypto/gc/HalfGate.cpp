...
std::vector<WireLabel> Garbler::inputOfServer(const BitVector &input) {
  std::vector<WireLabel> serverInput;
  const size_t input_size = input.size();
  serverInput.reserve(input_size);

  for (auto it = input.begin(); it != input.end(); ++it) {
    serverInput.emplace_back(rnd.randBlock());
    if (*it)
      (serverInput.back() ^ R).send(channel);
    else
      serverInput.back().send(channel);
  }
  return serverInput;  // Garbler needs the 0-Labels for garbling, not the actual
                      // input
}

std::vector<WireLabel> Garbler::inputOfClient(const size_t size) {
  std::vector<WireLabel> clientInput;
  clientInput.reserve(size);
  doOTPhase(size);

  for (size_t idx = 0; idx < size; idx++) {
    clientInput.emplace_back(OTs[idx][0]);
  }
  return clientInput;  // Garbler needs the 0-Labels for garbling, not the actual
                    // input
}

void Garbler::outputToClient(const std::vector<WireLabel> &outputLabels) {
  BitVector bv;
...
std::vector<WireLabel> Evaluator::inputOfServer(const size_t size) {
  std::vector<WireLabel> serverInput;
  serverInput.reserve(size);

  for (size_t idx = 0; idx < size; idx++) {
    serverInput.push_back(WireLabel::recv(channel));
  }
  return serverInput;
}

std::vector<WireLabel> Evaluator::inputOfClient(const BitVector &input) {
  std::vector<WireLabel> clientInput;
  const size_t input_size = input.size();
  clientInput.reserve(input_size);
  doOTPhase(input);

  for (size_t idx = 0; idx < input_size; idx++) {
    clientInput.push_back(WireLabel(OTs[idx]));
  }
  return clientInput;
}

BitVector Evaluator::outputToClient(const std::vector<WireLabel> &outputLabels) {
...
std::vector<WireLabel> SIMDGarbler::inputOfServer(const BitVector &input) {
  std::vector<WireLabel> serverInput;
  const size_t input_size = input.size();
  serverInput.reserve(input_size);
...
std::vector<SIMDWireLabel> SIMDGarbler::inputOfServer(
    const std::vector<BitVector> &input) {
  std::vector<SIMDWireLabel> serverInput;
  const size_t input_size = input.front().size();
  const size_t num_input = input.size();
  serverInput.reserve(input_size);

  for (size_t i = 0; i < input_size; i++) {
    serverInput.emplace_back(rnd.randBlocks(SIMDInputs));
    SIMDWireLabel tmp(serverInput.back());
...
std::vector<SIMDWireLabel> SIMDGarbler::inputOfClient(const size_t size) {
  std::vector<SIMDWireLabel> clientInput;
  clientInput.reserve(size);
...
void SIMDGarbler::outputToClient(const std::vector<SIMDWireLabel> &outputLabels) {
...
std::vector<WireLabel> SIMDEvaluator::inputOfServer(const size_t size) {
  std::vector<WireLabel> serverInput;
  serverInput.reserve(size);
...
std::vector<SIMDWireLabel> SIMDEvaluator::inputOfServerSIMD(const size_t size) {
  std::vector<SIMDWireLabel> serverInput;
  serverInput.reserve(size);

  for (size_t idx = 0; idx < size; idx++) {
    serverInput.push_back(SIMDWireLabel::recv(channel, SIMDInputs));
  }
  return serverInput;
}

std::vector<SIMDWireLabel> SIMDEvaluator::inputOfClient(
    const std::vector<BitVector> &input) {
  std::vector<SIMDWireLabel> clientInput;
  const size_t input_size = input.front().size();
  const size_t num_input = input.size();
...
std::vector<BitVector> SIMDEvaluator::outputToClient(
...
std::vector<WireLabel> SIMDGarblerPhases::inputOfServer(const BitVector &input) {
  std::vector<WireLabel> serverInput;
  const size_t input_size = input.size();
  serverInput.reserve(input_size);
  ...
  for (auto it = input.begin(); it != input.end(); ++it) {
    serverInput.emplace_back(rnd.randBlock());
    if (*it)
      (serverInput.back() ^ R).send(bufChan);
    else
      serverInput.back().send(bufChan);
  }
  return serverInput;  // Garbler needs the 0-Labels for garbling, not the actual
                      // input
}

std::vector<WireLabel> SIMDEvaluatorPhases::inputOfServer(const size_t size) {
  std::vector<WireLabel> serverInput;
  serverInput.reserve(size);

  for (size_t idx = 0; idx < size; idx++) {
    serverInput.push_back(WireLabel::recv(bufChan));
  }
  return serverInput;
}

std::vector<SIMDWireLabel> SIMDGarblerPhases::inputOfClientOffline(
    const size_t size) {
  clientInputLabels.clear();
  clientInputLabels.reserve(size);

  for (size_t idx = 0; idx < size; idx++) {
    clientInputLabels.emplace_back(rnd.randBlocks(SIMDInputs));
  }
  // Garbler needs the 0-Labels for garbling, not the actual input
  return clientInputLabels;
}

void SIMDGarblerPhases::inputOfClientOnline() {
  uint64_t size = clientInputLabels.size();
  BitVector all;
  {
  ...
  for (size_t idx = 0; idx < size; idx++) {
    std::vector<block> tmp(clientInputLabels[idx].bytes);
    for (size_t input_idx = 0; input_idx < SIMDInputs; input_idx++) {
...
std::vector<SIMDWireLabel> SIMDEvaluatorPhases::inputOfClientOnline(
  ...
  for (size_t idx = 0; idx < input_size; idx++) {
    ...
    clientInput.emplace_back(b);
  }
  return clientInput;
}

void SIMDGarblerPhases::outputToClient(
    const std::vector<SIMDWireLabel> &outputLabels) {
  for (const SIMDWireLabel &label : outputLabels) {
    BitVector bv = label.getLSB();
...
std::vector<BitVector> SIMDEvaluatorPhases::outputToClient(
    const std::vector<SIMDWireLabel> &outputLabels) {
  std::vector<BitVector> output(SIMDInputs);
  for (BitVector &bv : output) bv.reserve(outputLabels.size());
