...
#include <assert.h>
#include <droidCrypto/lowmc/endian_compat.h>
...
void Circuit::garble(const BitVector &inputA) {
  ...
  std::vector<WireLabel> serverInput = g.inputOfServer(inputA);
  std::vector<WireLabel> clientInput = g.inputOfClient(mInputB_size);

  auto time3 = std::chrono::high_resolution_clock::now();
  timeOT = time3 - time2;

  std::vector<WireLabel> outputs = computeFunction(serverInput, clientInput, g);

  auto time4 = std::chrono::high_resolution_clock::now();
  timeEval = time4 - time3;

  g.outputToClient(outputs);
...
BitVector Circuit::evaluate(const BitVector &inputB) {
  ...
  std::vector<WireLabel> serverInput = e.inputOfServer(mInputA_size);
  std::vector<WireLabel> clientInput = e.inputOfClient(inputB);

  auto time3 = std::chrono::high_resolution_clock::now();
  timeOT = time3 - time2;

  std::vector<WireLabel> outputs = computeFunction(serverInput, clientInput, e);
  ...
  BitVector output = e.outputToClient(outputs);
  auto time5 = std::chrono::high_resolution_clock::now();
  timeOutput = time5 - time4;
...
void SIMDCircuit::garble(const BitVector &inputA, const size_t SIMDvalues) {
  ...
  assert(inputA.size() == mInputA_size);
  std::vector<WireLabel> serverInput = g.inputOfServer(inputA);
  std::vector<SIMDWireLabel> clientInput = g.inputOfClient(mInputB_size);

  auto time3 = std::chrono::high_resolution_clock::now();
  timeOT = time3 - time2;

  std::vector<SIMDWireLabel> outputs = computeFunction(serverInput, clientInput, g);
...
void SIMDCircuit::garbleSIMD(const std::vector<BitVector> &inputA) {
  ...
  std::vector<SIMDWireLabel> serverInput = g.inputOfServer(inputA);
  std::vector<SIMDWireLabel> clientInput = g.inputOfClient(mInputB_size);
...
std::vector<BitVector> SIMDCircuit::evaluate(
  ...
  std::vector<SIMDWireLabel> clientInput = e.inputOfClient(inputB);
  ...
  std::vector<SIMDWireLabel> outputs = computeFunction(serverInput, clientInput, e);
  ...
  std::vector<BitVector> output = e.outputToClient(outputs);
  auto time5 = std::chrono::high_resolution_clock::now();
  timeOutput = time5 - time4;
...
std::vector<BitVector> SIMDCircuit::evaluateSIMD(
  ...
  assert(inputB.front().size() == mInputB_size);
  std::vector<SIMDWireLabel> serverInput = e.inputOfServerSIMD(mInputA_size);
  Log::v("GC", "inputA done");
...
void SIMDCircuitPhases::garbleBase(const BitVector &inputA,
  ...
  std::vector<WireLabel> serverInput = g->inputOfServer(inputA);
  std::vector<SIMDWireLabel> clientInput = g->inputOfClientOffline(mInputB_size);
  std::vector<SIMDWireLabel> outputs =
      computeFunction(serverInput, clientInput, *g);
  g->outputToClient(outputs);
  auto time4 = std::chrono::high_resolution_clock::now();
  timeEval = time4 - time3;
...
std::vector<BitVector> SIMDCircuitPhases::evaluateOnline(
  ...
  std::vector<SIMDWireLabel> clientInput =
      e->inputOfClientOnline(inputB, randChoices_);

  //        Log::v("GC", "inputB done");

  std::vector<SIMDWireLabel> outputs =
      computeFunction(serverInput, clientInput, *e);
