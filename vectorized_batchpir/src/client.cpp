...
Client::Client(PirParams &pir_params) : pir_params_(pir_params)
{
    ...
    initialize_query_ciphertext_wire_bytes();
}
...
void Client::initialize_query_ciphertext_wire_bytes()
{
    std::vector<uint64_t> encrypted_zero(polynomial_degree_, 0ULL);
    seal::Plaintext pt;
    batch_encoder_->encode(encrypted_zero, pt);
    query_ciphertext_wire_bytes_ = static_cast<std::uint64_t>(
        encryptor_->encrypt_symmetric(pt).save_size()
    );
}

std::uint64_t Client::query_ciphertext_wire_bytes() const
{
    return query_ciphertext_wire_bytes_;
}
