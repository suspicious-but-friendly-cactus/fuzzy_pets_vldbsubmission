...
bool BatchPIRClient::cuckoo_insert(uint64_t key, size_t attempt, std::unordered_map<uint64_t, std::vector<size_t>> key_to_buckets, std::unordered_map<uint64_t, uint64_t> &bucket_to_key)
{
    ...
    return cuckoo_insert(old, attempt + 1, key_to_buckets, bucket_to_key);
}
...
vector<PIRQuery> BatchPIRClient::create_queries(vector<uint64_t> batch)
{
    ...
    for (int i = 0; i < client_list_.size(); i++)
    {
        ...
        last_query_wire_bytes_ += static_cast<std::uint64_t>(query.size()) *
                                  client_list_[i].query_ciphertext_wire_bytes();
...
bool BatchPIRClient::cuckoo_hash(vector<uint64_t> batch)
{
    ...
    bool placed_without_failures = false;
    const uint64_t seed_base = static_cast<uint64_t>(
        std::chrono::high_resolution_clock::now().time_since_epoch().count()
    );
    for (size_t placement_attempt = 0; placement_attempt < 128 && !placed_without_failures; ++placement_attempt)
    {
        cuckoo_table_.assign(
            std::ceil(batchpir_params_.get_batch_size() * batchpir_params_.get_cuckoo_factor()),
            batchpir_params_.get_default_value()
        );
        bucket_to_key.clear();
        srand(static_cast<unsigned int>(seed_base + placement_attempt));

        placed_without_failures = true;
        for (auto const &[key, value] : key_to_buckets)
        {
            placed_without_failures =
...
std::uint64_t BatchPIRClient::last_query_wire_bytes() const
{
    return last_query_wire_bytes_;
}
...
vector<uint64_t> BatchPIRClient::get_request_table()
{
...
void BatchPIRClient::translate_cuckoo()
{
    ...
    for (int i = 0; i < num_buckets; i++)
    {
        ...
        if (cuckoo_table_[i] != batchpir_params_.get_default_value())
        {
            ...
            const std::string map_key = to_string(cuckoo_table_[i]) + ":" + to_string(i);
            auto found = map_.find(map_key);
            if (found == map_.end())
            {
                throw std::runtime_error("BatchPIRClient: missing server hash-map entry for requested row");
            }
            cuckoo_table_[i] = found->second;
        }
    }
}
...
void BatchPIRClient::prepare_pir_clients()
{
    ...
    if (dim_size == 0)
    {
        throw std::invalid_argument(
            "BatchPIR single-server protocol requires first_dimension_size to be set"
        );
    }
    ...
    {
        throw std::invalid_argument(
            "BatchPIR single-server protocol has zero internal client capacity"
        );
    }
    ...
    {
        throw std::invalid_argument(
            "BatchPIR single-server protocol requires all buckets to fit in one internal PIR client"
        );
    }
