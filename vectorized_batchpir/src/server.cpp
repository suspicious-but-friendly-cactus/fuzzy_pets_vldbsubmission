...
bool is_transparent_error(const std::logic_error &e)
{
    return std::string(e.what()).find("transparent") != std::string::npos;
}
...
size_t Server::encrypted_zero_query_index() const
{
    return pir_dimensions_.size();
}

seal::parms_id_type Server::next_parms_id(seal::parms_id_type parms_id) const
{
    auto context_data = context_->get_context_data(parms_id);
    if (!context_data || !context_data->next_context_data())
    {
        throw std::runtime_error("BatchPIR Server: no next SEAL parms level is available");
    }
    return context_data->next_context_data()->parms_id();
}

seal::Ciphertext Server::encrypted_zero_at(seal::parms_id_type parms_id, bool ntt_form)
{
    const size_t zero_idx = encrypted_zero_query_index();
    if (query_.size() <= zero_idx)
    {
        throw std::runtime_error("BatchPIR Server: query is missing encrypted zero helper ciphertext");
    }

    seal::Ciphertext zero = query_[zero_idx];
    if (zero.parms_id() != parms_id)
    {
        evaluator_->mod_switch_to_inplace(zero, parms_id);
    }
    if (ntt_form && !zero.is_ntt_form())
    {
        evaluator_->transform_to_ntt_inplace(zero);
    }
    return zero;
}

seal::Ciphertext Server::encrypted_zero_like(const seal::Ciphertext& like)
{
    return encrypted_zero_at(like.parms_id(), like.is_ntt_form());
}

void Server::replace_transparent_with_zero(seal::Ciphertext& ct)
{
    if (ct.is_transparent())
    {
        ct = encrypted_zero_like(ct);
    }
}

void Server::add_inplace_or_zero(seal::Ciphertext& acc, const seal::Ciphertext& term)
{
    try
    {
        evaluator_->add_inplace(acc, term);
        replace_transparent_with_zero(acc);
    }
    catch (const std::logic_error &e)
    {
        if (!is_transparent_error(e))
        {
            throw;
        }
        acc = encrypted_zero_like(acc);
    }
}
...
PIRResponseList Server::merge_responses_chunks_buckets(vector<PIRResponseList> &responses, uint32_t client_id)
{
    ...
    for (int i = 0; i < responses.size(); i++)
    {
        ...
        for (int j = 0; j < num_chunk_ctx; j++)
        {
            ...
            replace_transparent_with_zero(chunk_ct_acc);
            for (size_t k = 1; k < loop; k++)
            {
                replace_transparent_with_zero(responses[i][chunk_idx + k]);
                evaluator_->rotate_rows_inplace(responses[i][chunk_idx + k], -1 * (k * gap_), client_keys_[client_id].first);
                replace_transparent_with_zero(responses[i][chunk_idx + k]);
                add_inplace_or_zero(chunk_ct_acc, responses[i][chunk_idx + k]);
            }
    ...
    for (int i = 0; i < merged_ctx_needed; i++)
    {
        ...
        for (int j = 0; j < num_buckets_merged; j++)
        {
            ...
            replace_transparent_with_zero(copy_ct_acc);
            ...
            for (size_t k = 1; k < row_size_ / current_fill; k *= 2)
            {
                ...
                replace_transparent_with_zero(tmp_ct);
                add_inplace_or_zero(copy_ct_acc, tmp_ct);
                tmp_ct = copy_ct_acc;
            }
            ...
            catch (const std::logic_error &e)
            {
                ...
                copy_ct_acc = encrypted_zero_like(copy_ct_acc);
            }
            if (j == 0)
            {
                ct_acc = copy_ct_acc;
            ...
            {
                add_inplace_or_zero(ct_acc, copy_ct_acc);
            }
        }
...
void Server::modulus_switch(PIRResponseList& list){
    for( int i = 0; i < list.size(); i++){
        const auto target_parms = next_parms_id(list[i].parms_id());
        if (list[i].is_transparent())
        {
            list[i] = encrypted_zero_at(target_parms, list[i].is_ntt_form());
            continue;
        }
        ...
        try
        {
            ...
            replace_transparent_with_zero(list[i]);
        }
        catch (const std::logic_error &e)
        {
            if (!is_transparent_error(e))
...
PIRResponseList Server::merge_responses_buckets_chunks(vector<PIRResponseList> &responses, uint32_t client_id)
{
    ...
    for (int j = 0; j < responses[0].size(); j++)
    {
        ...
        for (int i = 0; i < responses.size(); i++)
        {
            ...
            replace_transparent_with_zero(ct_acc);
            ...
            for (size_t k = 1; k < row_size_ / gap_; k *= 2)
            {
                ...
                replace_transparent_with_zero(ct);
                add_inplace_or_zero(ct_acc, ct);
                ct = ct_acc;
            }
            ...
            {
                add_inplace_or_zero(bucket_ct_acc, ct);
            }
        }
    ...
    for (int i = 0; i < ciphertext_needed; i++)
    {
        ...
        {
            replace_transparent_with_zero(bucket_response[j + (i * capacity)]);
            evaluator_->rotate_rows_inplace(bucket_response[j + (i * capacity)], -1 * i * gap_, client_keys_[client_id].first);
            replace_transparent_with_zero(bucket_response[j + (i * capacity)]);
            add_inplace_or_zero(chunk_ct_acc, bucket_response[j + (i * capacity)]);
        }
...
vector<Ciphertext> Server::process_first_dimension_delayed_mod(uint32_t client_id)
{
    ...
    for (int col_id = 0; col_id < encoded_db_.size(); col_id += pir_dimensions_[1])
    {
        ...
        if (ct_acc.is_transparent())
        {
            ct_acc = encrypted_zero_at(ct_acc.parms_id(), false);
        }
        else
        {
            try
...
vector<Ciphertext> Server::process_second_dimension(uint32_t client_id, vector<Ciphertext> first_intermediate_data)
{
    ...
    const seal::parms_id_type second_level_parms = next_parms_id(query_[1].parms_id());
    
    for (int idx = 0; idx < first_intermediate_data.size(); idx += pir_dimensions_[2])
    {

        Ciphertext ct_acc = encrypted_zero_at(second_level_parms);

        for (int i = 0; i < pir_dimensions_[2]; i += 1)
        {
            Ciphertext ct1;
            try
            {
                ...
                replace_transparent_with_zero(ct1);
            }
            catch (const std::logic_error &e)
            {
                if (!is_transparent_error(e))
                ...
                ct1 = encrypted_zero_at(second_level_parms);
            }
            ...
            add_inplace_or_zero(ct_acc, ct1);
        }
...
PIRResponseList Server::process_last_dimension(uint32_t client_id, vector<Ciphertext> second_intermediate_data, bool is_2d_pir_)
{
    ...
    Ciphertext last_query = query_[pir_dimensions_.size() - 1];
    if(!is_2d_pir_){
        evaluator_->mod_switch_to_next_inplace(last_query);
    }
    for (int idx = 0; idx < second_intermediate_data.size(); idx++)
    {
        ...
        try
        {
            evaluator_->multiply(last_query, second_intermediate_data[idx], ct);
        ...
        catch (const std::logic_error &e)
        {
            ...
            ct = encrypted_zero_at(last_query.parms_id());
        }
