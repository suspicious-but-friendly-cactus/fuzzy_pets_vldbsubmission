...
BatchPIRServer::BatchPIRServer(BatchPirParams &batchpir_params, RawDB rawdb)
{
    batchpir_params_ = &batchpir_params;
    is_client_keys_set_ = false;
    is_simple_hash_ = false;

    if (rawdb.size() != batchpir_params_->get_num_entries())
    {
        throw std::invalid_argument("BatchPIRServer: raw database entry count does not match parameters");
    }
    for (const auto &entry : rawdb)
    {
        if (entry.size() != batchpir_params_->get_entry_size())
        {
            throw std::invalid_argument("BatchPIRServer: raw database entry size does not match parameters");
        }
    }

    rawdb_ = std::move(rawdb);

    std::cout << "BatchPIRServer: Using caller-provided raw database." << std::endl;
    std::cout << "BatchPIRServer: Performing simple hash and bucket balancing..." << std::endl;
    simeple_hash();
    balance_buckets();
    std::cout << "BatchPIRServer: Simple hash and balancing completed." << std::endl;
...
void BatchPIRServer::prepare_pir_server()
{
    ...
    {
        throw std::invalid_argument(
            "BatchPIR single-server protocol has zero internal server capacity"
        );
    }
    ...
    {
        throw std::invalid_argument(
            "BatchPIR single-server protocol requires all buckets to fit in one internal PIR server"
        );
    }
