...
private:
    ...
    size_t encrypted_zero_query_index() const;
    seal::parms_id_type next_parms_id(seal::parms_id_type parms_id) const;
    seal::Ciphertext encrypted_zero_at(seal::parms_id_type parms_id, bool ntt_form = false);
    seal::Ciphertext encrypted_zero_like(const seal::Ciphertext& like);
    void replace_transparent_with_zero(seal::Ciphertext& ct);
    void add_inplace_or_zero(seal::Ciphertext& acc, const seal::Ciphertext& term);
